...
test('apiSearchIssuesPath returns /issues?jql=<text for search>', t => {
  ...
  const path = apiSearchIssuesPath("this doesn't look like anything to me", { fields: 'status', maxResults: 5 });
  const searchString = encodeURIComponent(`text ~ "this doesn't look like anything to me" AND status!="Done" ORDER BY lastViewed DESC`);
