...
import { apiIssuePath, apiTransitionsPath } from '../../../jira/paths.js';
...
class JiraBar extends React.Component {
      ...
      jiraClient
        ...
        .then(issue => {
          this.setState({ issue, transitioning: false }, () => {
            jiraClient
              .get(apiTransitionsPath, ticket)
              .then(({ transitions }) => {
                this.setState({
                  transitions: uniqBy(transitions, 'id').slice(-2)
                });
              });
          });
        });
  ...
  transitionCard({ id }) {
    ...
    if (!transitioning && ticket) {
      this.setState({ transitioning: true }, () => {
        jiraClient
          .post(apiTransitionsPath(ticket), { transition: { id } })
          .then(response => {
  ...
  render() {
    ...
    const { fields: { summary, status, assignee } = {} } = issue;

    const description = issue.fields.description || '';

    const { name: assignedUserName, avatarUrls: { '48x48': avatarUrl } = {} } =
      assignee || {};
      ...
      <div className="jira-bar" style={baseStyles}>
        <div style={{ ...headingStyles, ...commonSpacing }}>
          ...
          <span title={summary} style={{ ...truncate, ...cardTitleStyles }}>
            <strong>
              <a href={jiraClient.issueURL(issue.key)} target="_blank">
                {issue.key}
              </a>
              :
            </strong>{' '}
          ...
          <ul style={transitionListStyles}>
            {transitions.map(transition => (
              <TransitionListItem
                ...
                styles={{
                  ...transitionListItemStyles,
                  ...statusStyles,
                  ...truncate,
                  ...(transitioning ? transitioningListItemStyles : {})
                }}
        ...
        {bodyExpanded && (
          <div style={{ ...commonSpacing, ...bodyStyles }}>
            ...
            />
            <a href="#" onClick={this.toggleExpandHTML}>
              - Show summary{' '}
            </a>
          </div>
        )}
        ...
        {!bodyExpanded && (
          <div style={{ ...commonSpacing, ...bodyStyles }}>
            <p>
              ...
              {description.length > SHORT_DESCRIPTION_LENGTH ? '...' : null}
            </p>
            <a href="#" onClick={this.toggleExpandHTML}>
              + Show full HTML
            </a>
